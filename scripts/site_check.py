...
def check_css(css, where, tokens, report, line_offset=0, is_file=True):
    for sel, prop, value, line, ignored in iter_declarations(css):
        ...
        shown = "(inline style)" if sel == "__inline__" else sel
        if ignored:
            continue
        ...
        if prop in ("outline", "outline-style", "outline-width") and re.match(r"^(none|0(px)?)\b", value) \
            ...
            report.error("3", "Focus ring removed", loc, f"{shown} -> {prop}: {value}")
    ...
    for sel, prop, value, _, ignored in iter_declarations(css):
        ...
        for part in sel.split(","):
            ...
            if not last or re.search(r"[:\[]", last):
                continue    # hover/focus states, pseudo-elements, attribute selectors aren't surfaces
            tag = re.match(r"^[a-zA-Z][\w-]*", last)
            tag = tag.group(0).lower() if tag else None
            classes, ids = frozenset(re.findall(r"\.([\w-]+)", last)), frozenset(re.findall(r"#([\w-]+)", last))
            if (tag or classes or ids) and tag not in NOT_SURFACES:
                out.append((tag, classes, ids))
    return out
...
def block_signature(raw, tag):
    m = re.search(rf"<{tag}\b[^>]*>.*?</{tag}>", raw, re.S | re.I)
    if not m:
        return None
    s = m.group(0)
    s = re.sub(r'\saria-current="[^"]*"', "", s)
    s = re.sub(r'class="([^"]*)"', lambda c: 'class="' + " ".join(
        w for w in c.group(1).split() if w not in ("active", "is-active", "current", "is-current")) + '"', s)
...
def check_variants(pages, tag, phase, label, report):
    ...
    for p, (_, raw) in pages.items():
        sig = block_signature(raw, tag)
        if sig:
            groups[sig].append(os.path.basename(p))
    if len(groups) > 1:
        ordered = sorted(groups.values(), key=len, reverse=True)
        desc = "; ".join(
            f"variant {chr(65 + i)} ({len(g)} page{'s' if len(g) > 1 else ''}: {', '.join(sorted(g)[:6])}"
            f"{', ...' if len(g) > 6 else ''})" for i, g in enumerate(ordered))
